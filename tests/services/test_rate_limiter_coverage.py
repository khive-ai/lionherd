...
from lionherd.services.utilities.rate_limiter import RateLimitConfig, TokenBucket
...
class TestTokenBucket:
    ...
    async def test_to_dict(self):
        ...
        assert result["tokens"] >= 7.0
        assert "last_refill" in result
        assert isinstance(result["last_refill"], float)
