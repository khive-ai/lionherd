...
import pytest
from lionherd_core import Event, EventStatus

from lionherd.services.types.hook import (
    HookBroadcaster,
    HookEvent,
    HookPhase,
    HookRegistry,
...
class MockEvent(Event):
    """Mock event for testing hooks."""

    test_value: str = "test"

    async def _invoke(self):
        """Mock invoke."""
        self.execution.response = f"Invoked: {self.test_value}"
...
async def test_registry_pre_event_create_when_hook_returns_event_then_success():
    ...
    async def hook(event_type, **kw):
        ...
        return MockEvent(test_value="from_hook")
    ...
    assert called_with["event_type"] == MockEvent
...
async def test_registry_pre_event_create_when_hook_returns_none_then_success():
    ...
    result, should_exit, status = await registry.pre_event_create(MockEvent)
...
async def test_registry_pre_event_create_when_hook_raises_and_exit_true():
    ...
    result, should_exit, status = await registry.pre_event_create(MockEvent, exit=True)
...
async def test_hook_event_invoke_when_hook_returns_tuple_with_exception():
    ...
    from lionherd_core import Undefined
...
async def test_hook_event_invoke_when_hook_returns_exception():
    ...
    assert hook_event.execution.error == "Direct exception"
...
async def test_registry_internal_call_when_no_hook_phase_and_no_chunk_then_raises():
    ...
    with pytest.raises(RuntimeError, match="Either hook_type or chunk_type must be provided"):
        await registry._call(None, None, None, MockEvent)
...
async def test_registry_internal_call_when_hook_phase_missing_chunk_type_then_raises():
    ...
    # hp_ exists but not in registry, ct_ is None -> RuntimeError (line 187)

    # This is hard to trigger because line 182 checks if hp_ and registry has it
    # If hp_ is not in registry, line 187 raises RuntimeError
    # But we need hp_ to be truthy but not in registry

    # Actually, line 182: if hp_ and (self._hooks.get(hp_)):
    # If hp_ is truthy but not in _hooks, we skip to elif not ct_ (line 186)
    # Then raise RuntimeError (line 187)
